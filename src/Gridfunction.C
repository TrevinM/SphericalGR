...
// Constructor
//================================================
gf3d::gf3d() : nr(0), nt(0), np(0), v(NULL), center_par(1),
axis_par(1), eq_par(1), PI(acos(-1.0)) {
};
...
// 
gf3d* gf3d::setup(Grid* grid_i, int fall_off_i, const char* name_i,
...
// 
gf3d* gf3d::setup(Grid* grid_i, int fall_off_i, const char* name_i, int gfn_i) {
    gf3d* address = setup(grid_i, fall_off_i);
    gfn = gfn_i;
    name = name_i;
    return address;
...
// 
gf3d* gf3d::setup(Grid* grid_i, int fall_off_i, int gfn_i) {
    gf3d* address = setup(grid_i, fall_off_i);
    gfn = gfn_i;
    return address;
}
...
// Minimum version
//
gf3d* gf3d::setup(Grid* grid_i, int fall_off_i) {
    grid = grid_i;
    gfn = 0;
    N_g = grid->N_ghosts();
    nr = grid->N_r_tot();
...
//================================================
int gf3d::fill_outerboundary(gf3d& fct_old, double dt, double update_background) {
    background = update_background;
    fill_outerboundary(fct_old, dt);
    return 1;
}

int gf3d::fill_outerboundary(gf3d& fct_old, double dt) {
    ...
    //
    for (int i = nr - 1; i >= nr - N_g; i--) {
        double Courant = wave_speed * dt / delta_x * dxdr(i);
        double r0 = (*r_p)[i - 2];
        double r1 = (*r_p)[i - 1];
        double r2 = (*r_p)[i];
        double r_last = Courant * r1 + (1.0 - Courant) * r2; // intersection
        ...
        for (int j = 0; j < nt; j++)
            for (int k = 0; k < np; k++) {
                ...
                const double P1 = fct_old[i - 1][j][k];
                const double P2 = fct_old[i][j][k];
    ...
    // for (int j = 0; j < nt; j++) 
    //   for (int k = 0; k < np; k++) {	  
    // 	const double P0 = fct_old[nr-4][j][k];
    // 	const double P1 = fct_old[nr-3][j][k];
    // 	const double P2 = fct_old[nr-2][j][k];
    // 	// Now use Neville's algorithm...
    // 	const double P01  = ( (r_last - r1) * P0  + (r0 - r_last) * P1  ) / (r0 - r1);
    // 	const double P12  = ( (r_last - r2) * P1  + (r1 - r_last) * P2  ) / (r1 - r2);
    // 	const double P012 = ( (r_last - r2) * P01 + (r0 - r_last) * P12 ) / (r0 - r2);
    ...
    // for (int j = 0; j < nt; j++) 
    //   for (int k = 0; k < np; k++) {	  
    // 	const double P0 = fct_old[nr-5][j][k];
    // 	const double P1 = fct_old[nr-4][j][k];
    // 	const double P2 = fct_old[nr-3][j][k];
    // 	// Now use Neville's algorithm...
    // 	const double P01  = ( (r_last - r1) * P0  + (r0 - r_last) * P1  ) / (r0 - r1);
    // 	const double P12  = ( (r_last - r2) * P1  + (r1 - r_last) * P2  ) / (r1 - r2);
    // 	const double P012 = ( (r_last - r2) * P01 + (r0 - r_last) * P12 ) / (r0 - r2);
...
//
Doub gf3d::Laplace(int i, int j, int k) {
...
//
Doub gf3d::Laplace_so(int i, int j, int k) {
...
// first derivatives
// 
Doub gf3d::dr(int i, int j, int k) {
#ifdef EIGHTHORDER
...
Doub gf3d::dtheta(int i, int j, int k) {
#ifdef EIGHTHORDER
...
Doub gf3d::dphi(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
...
// second derivatives
//
Doub gf3d::ddr(int i, int j, int k) {
#ifdef EIGHTHORDER
...
Doub gf3d::ddtheta(int i, int j, int k) {
#ifdef EIGHTHORDER
...
Doub gf3d::ddphi(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
...
//  
Doub gf3d::dthetadr(int i, int j, int k) { return drdtheta(i, j, k); }
Doub gf3d::drdtheta(int i, int j, int k) {
#ifdef EIGHTHORDER
    return (
...
Doub gf3d::dphidr(int i, int j, int k) { return drdphi(i, j, k); }
Doub gf3d::drdphi(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
...
Doub gf3d::dphidtheta(int i, int j, int k) { return dthetadphi(i, j, k); }
Doub gf3d::dthetadphi(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
...
//
Doub gf3d::dr_so(int i, int j, int k) {
    return (v[i + 1][j][k] - v[i - 1][j][k]) / (2.0 * delta_x) * dxdr(i);
}
Doub gf3d::dtheta_so(int i, int j, int k) {
    return (v[i][j + 1][k] - v[i][j - 1][k]) / (2.0 * delta_y) * dydtheta(j);
}
Doub gf3d::dphi_so(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
...
//
Doub gf3d::dthetadr_so(int i, int j, int k) { return drdtheta_so(i, j, k); }
Doub gf3d::drdtheta_so(int i, int j, int k) {
    return (v[i + 1][j + 1][k] - v[i + 1][j - 1][k] - v[i - 1][j + 1][k] + v[i - 1][j - 1][k])
        / (4.0 * delta_x * delta_y) * dxdr(i) * dydtheta(j);
}
Doub gf3d::dphidr_so(int i, int j, int k) { return drdphi_so(i, j, k); }
Doub gf3d::drdphi_so(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
...
Doub gf3d::dphidtheta_so(int i, int j, int k) { return dthetadphi_so(i, j, k); }
Doub gf3d::dthetadphi_so(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
...
// 
Doub gf3d::dr(int i, int j, int k, double shift) {
    if (shift < 0.0) {
#if defined SIXTHORDER || defined EIGHTHORDER
...
Doub gf3d::dtheta(int i, int j, int k, double shift) {
    if (shift < 0.0) {
#if defined SIXTHORDER || defined EIGHTHORDER
...
Doub gf3d::dphi(int i, int j, int k, double shift) {
#ifdef AXISYMMETRY
    return 0.0;
#else
    if (shift < 0.0)
...
//
Doub gf3d::dr_to(int i, int j, int k, double shift) {
    if (shift < 0.0)
...
Doub gf3d::dtheta_to(int i, int j, int k, double shift) {
    if (shift < 0.0)
...
Doub gf3d::dphi_to(int i, int j, int k, double shift) {
#ifdef AXISYMMETRY
    return 0.0;
#else
    if (shift < 0.0)
...
//
Doub gf3d::dr_OS(int i, int j, int k, double shift) {
    // if (shift < 0.0) 
...
Doub gf3d::d4(int i, int j, int k) {
...
//
Doub gf3d::D8r(int i, int j, int k) {
...
Doub gf3d::D8theta(int i, int j, int k) {
    const double zero = 70.0;
    const double one = -56.0;
    const double two = 28.0;
    const double three = -8.0;
...
Doub gf3d::D8phi(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
    const double zero = 70.0;
...
//
Doub gf3d::D6r(int i, int j, int k) {
...
Doub gf3d::D6theta(int i, int j, int k) {
    const double zero = -20.0;
    const double one = 15.0;
    const double two = -6.0;
    const double three = 1.0;
...
Doub gf3d::D6phi(int i, int j, int k) {
#ifdef AXISYMMETRY
    return 0.0;
#else
    const double zero = -20.0;
...
Doub gf3d::KO(int i, int j, int k) {
...
double gf3d::min(int& i_min, int& j_min, int& k_min) {
    double min = v[N_g][N_g][N_g];
...
double gf3d::max(int& i_max, int& j_max, int& k_max) {
    double max = v[N_g][N_g][N_g];
...
double gf3d::abs_max(int& i_max, int& j_max, int& k_max) {
    double max = abs(v[N_g][N_g][N_g]);
    i_max = j_max = k_max = N_g;
    for (int i = N_g; i < nr - N_g; i++)
        for (int j = N_g; j < nt - N_g; j++)
...
double gf3d::max(double r_min) {   // finds maximum for all r > r_min
...
// 
double gf3d::radialmax(double theta, int k, double& r_max) {
...
double gf3d::radialmax(int j, int k, double& r_max) {
...
// 
double gf3d::slicemax(int k) {
    double r_max, theta_max;
    return slicemax(k, r_max, theta_max);
}
double gf3d::slicemax(int k, double& r_max, double& theta_max) {
    //
...
// 
double gf3d::max(double x_0, double x_1, double x_2, double f_0, double f_1, double f_2, double& x_max) {
...
// 
double gf3d::radialmin(double theta, int k, double& r_min) {
...
double gf3d::center(int j, int k) {
    if (center_par != 1) {
...
// 
double gf3d::L2_norm(double r_frac) {
    double norm = 0.0;
    double vol_int = 0.0;
    for (int i = N_g; i < nr - N_g; i++) {
        double rl = (*r_p)[i];
...
//================================================
Doub gf3d::operator()(double rl, int j, int k, int ilo) {
...
//================================================
Doub gf3d::operator()(int i, double theta, int k) {
...
//================================================
Doub gf3d::operator()(double rl, double theta, int k,
    int loc_order, int ilo) {
...
//================================================
//
Doub gf3d::operator()(int i, int j, double phi) {
    // first find klo
...
// surface integral
//================================================
Doub gf3d::surface_integral(int i) {
    double integral = 0.0;
...
//================================================
bool gf3d::checkpoint(int timestep) {
