...
// Notes:
//
// - measure T in units of m_e, i.e. tau \equiv k_B T / m_e
// - measure entropy s in units of k_B (up to constant only)
// - rescale everything else with respect to variable "length"
// - radiation constant:
//        alpha = K^3 a m_e^4 / k_B^4
// - then entropy:
//        s_r = 4 alpha (m_B / m_e) tau^3 / (3 rho_0)
//        s_g = (1/mu) * ln( tau^(3/2) / rho_0 ) + const
//   where
//================================================
//
class gas_radiation : public EOS {
private:
    double s_init = 0.0;  // initial entroy s_bar, up to constant (units of k_B)
    double X = 0.0;       // hydrogen mass fraction
    double Y = 0.0;       // Y = 1 - X -- i.e. assume zero metalicity
    double mu = 0.0;      // mean molecular weight
    const double m_e_cm = 6.764e-56;  // see MTW, in units of cm
    const double m_B_cm = 1.2419e-52;
    const double me_o_mB = m_e_cm / m_B_cm;
    const double PI = acos(-1.0);
    const double h_cm = 2. * PI * 2.612e-66;  // h in units of cm^2 (NOT hbar...)
    const double G = 6.67e-8;  // G in cgs units
    const double c = 2.99e10;
    const double M_sun = 2.e33;
    double length = 0.0;   // length scale
...
public:
    ...
    gas_radiation(char* input_file) : EOS(0.0) {
        ...
        if (!infile) {
            ...
            << input_file << endl;
        char buf[100], ccc;
        infile.get(buf, 100, '='); infile.get(ccc); infile >> X;
        infile.get(buf, 100, '='); infile.get(ccc); infile >> s_init;
        cout << " GASRAD: Setting up gas & radiation with X = " << X
            << " and s_init = " << s_init << endl;
        Y = 1.0 - X;                    // assume zero metallicity
        mu = 1.0 / (2. * X + 3. * Y / 4.);    // assume gas fully ionized
        // define length scale:
        length = 1.e6 * G * M_sun / (c * c);   // in cm
        // define constant K:
        ...
        name << "for gas and radiation mixture with X = "
            << X << " and length = " << length << " cm ";
        cout << " GASRAD: length = " << length << " cm" << endl;
        // define alpha \equiv a m_e^4 / k_B^4 (see Notes at top of file)
        const double m_o_h = m_e_cm / h_cm;
        const double m_o_h3 = m_o_h * m_o_h * m_o_h;
        const double alpha_cm = 8. * PI * PI * PI * PI * PI * m_o_h3 * m_e_cm / 15.;
        cout << " GASRAD: alpha = " << alpha_cm << " cm^{-2}" << endl;
        // and convert to code units and include eta:
        alpha = eta * length * length * alpha_cm;
        cout << " GASRAD: alpha = " << alpha << " (code units)" << endl;
        // now find constant for entropy (see Eq. (???) in ???)
        const double bar_h = h_cm / (length * length);
        const double bar_m_e = m_e_cm / length;
        const double bar_m_B = m_B_cm / length;
        const double temp1 = 3. * log(2. * PI / (bar_h * bar_h)) / (2. * mu) + 5. / (2. * mu);
        const double temp2 = (2. * X + 7. * Y / 4.) * log(2) - (X + Y / 2.) * log(X + Y / 2.);
        const double temp3 = -X * log(X) - Y * log(Y) / 4.;
        const double temp4 = (9. * X / 2. + 15. * Y / 8.) * log(bar_m_e) + (7. * X / 2. + 9. * Y / 8) * log(bar_m_B);
        s_const = temp1 + temp2 + temp3 + temp4;
        cout << " GASRAD: s_const = " << s_const << endl;
    ...
    double P(double rho_0, double epsilon, double tau) {
        ...
        while ((fabs(delta_eps) > tol * fabs(epsilon)) and (it < itmax)) {
            it++;
            const double depsdtau = d_eps_d_tau(rho_0, tau);
    ...
    double rho_0(double P) {
        ...
        while ((fabs(delta) > tol * s_init) and (it < itmax)) {
            it++;
            const double dPdtau = d_P_d_tau(rho_r, tau);
            const double dPdrho = d_P_d_rho(rho_r, tau);
            const double dsdtau = d_s_d_tau(rho_r, tau);
            const double dsdrho = d_s_d_rho(rho_r, tau);
        ...
        if (it >= itmax || !isfinite(delta)) {
            cout << "GASRAD: failed to converge in GasRad::rho_0 for P = "
                << P << " s = " << s_init << ", delta = " << delta << endl;
        }
        return rho_r;
    };
    //===============================================================
    ...
    double rho_0_of_rho(double rho) {
        ...
        while ((fabs(delta) > tol * s_init) and (it < itmax)) {
            it++;
            const double drhodtau = d_rho_d_tau(rho_r, tau);
            const double drhodrho_0 = d_rho_d_rho_0(rho_r, tau);
            const double dsdtau = d_s_d_tau(rho_r, tau);
            const double dsdrho = d_s_d_tau(rho_r, tau);
            const double det = drhodrho_0 * dsdtau - drhodtau * dsdrho;
            rho_r -= (dsdtau * delta_rho - drhodtau * delta_s) / det;
            tau -= (-dsdrho * delta_rho + drhodrho_0 * delta_s) / det;
            rho_r = fabs(rho_r);
            tau = fabs(tau);
    ...
    double cold_eps(double rho_0) {
        ...
        while ((fabs(delta_s) > tol * s_init) and (it < itmax)) {
            it++;
            const double dsdtau = d_s_d_tau(rho_0, tau);
            tau -= delta_s / dsdtau;
    ...
    double temperature(double rho_0, double epsilon) {
        if (rho_0 <= 0.0)
            ...
            const double beta3 = 3. * me_o_mB * rho_0 / (2. * mu * alpha);
    ...
    inline double press(double rho_0, double tau) {
        const double P_rad = alpha * tau * tau * tau * tau / 3.0;
        const double P_gas = rho_0 * tau * me_o_mB / mu;
        return P_rad + P_gas;
    };
    ...
    inline double eps(double rho_0, double tau) {
        const double eps_rad = alpha * tau * tau * tau * tau / rho_0;
        const double eps_gas = 3. * tau * me_o_mB / (2. * mu);
        return eps_rad + eps_gas;
    };
    inline double ent(double rho_0, double tau) {
        const double ent_rad = 4. * alpha * tau * tau * tau / (3. * me_o_mB * rho_0);
        const double ent_gas = log(tau * tau * tau / (rho_0 * rho_0)) / (2.0 * mu) + s_const;
    ...
    //
    //===============================================================    
    inline double d_eps_d_tau(double rho_0, double tau) {
        return 4. * alpha * tau * tau * tau / rho_0 + 3. * me_o_mB / (2. * mu);
    };
    inline double d_P_d_tau(double rho_0, double tau) {
        return 4. * alpha * tau * tau * tau / 3. + me_o_mB * rho_0 / mu;
    };
    inline double d_P_d_rho(double rho_0, double tau) {
        return me_o_mB * tau / mu;
    };
    inline double d_s_d_tau(double rho_0, double tau) {
        return 4. * alpha * tau * tau / (rho_0 * me_o_mB) + 3. / (2. * mu * tau);
    };
    inline double d_s_d_rho(double rho_0, double tau) {
        return -4. * alpha * tau * tau * tau / (3. * rho_0 * rho_0 * me_o_mB) - 1. / (mu * rho_0);
    };
    inline double d_rho_d_tau(double rho_0, double tau) {
        return 4. * alpha * tau * tau * tau + 3. * me_o_mB * rho_0 / (2. * mu);
    };
    inline double d_rho_d_rho_0(double rho_0, double tau) {
        return 1.0 + 3.0 * me_o_mB * tau / (2. * mu);
    };
    //===============================================================    
    //
    // Test...
