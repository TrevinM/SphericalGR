...
class ReadFromCheckPoint : public InData {
    ...
    gf3d sf, pi, e_p, a_r, a_t, a_p, as_r, as_t, as_p;
    ...
    bool sf_found, pi_found;
    bool e_p_found, a_r_found, a_t_found, a_p_found;
    bool as_r_found, as_t_found, as_p_found;
...
public:
    ...
    bool Initialize(gf3d& fct) {
        ...
        a_r.setup(grid, 1, "a_r", gf_counter++, -1, +1, +1);
        a_r_found = read_gf(&a_r, timestep);
        a_t.setup(grid, 1, "a_t", gf_counter++, +1, -1, -1);
        a_t_found = read_gf(&a_t, timestep);
        a_p.setup(grid, 1, "a_p", gf_counter++, -1, -1, +1);
        a_p_found = read_gf(&a_p, timestep);
        as_r.setup(grid, 1, "as_r", gf_counter++, -1, +1, +1);
        as_r_found = read_gf(&as_r, timestep);
        as_t.setup(grid, 1, "as_t", gf_counter++, +1, -1, -1);
        as_t_found = read_gf(&as_t, timestep);
        as_p.setup(grid, 1, "as_p", gf_counter++, -1, -1, +1);
        as_p_found = read_gf(&as_p, timestep);
        //
    ...
    double a_r_analytical(double r, double theta, double phi, double t) {
        if (a_r_found) {
            ...
            cerr << " INDATA: Cannot find checkpoint data for a_r -- aborting..."
                << endl;
            exit(0);
        }
        return 0.0;
    ...
    double a_t_analytical(double r, double theta, double phi, double t) {
        if (a_t_found) {
            ...
            cerr << " INDATA: Cannot find checkpoint data for a_t -- aborting..."
                << endl;
            exit(0);
        }
        return 0.0;
    ...
    double as_r_analytical(double r, double theta, double phi, double t) {
        if (as_r_found) {
            ...
            cerr << " INDATA: Cannot find checkpoint data for as_r -- aborting..."
                << endl;
            exit(0);
        }
        return 0.0;
    ...
    double as_t_analytical(double r, double theta, double phi, double t) {
        if (as_t_found) {
            ...
            cerr << " INDATA: Cannot find checkpoint data for as_t -- aborting..."
                << endl;
            exit(0);
        }
        return 0.0;
    ...
    double as_p_analytical(double r, double theta, double phi, double t) {
        if (as_p_found) {
            ...
            cerr << " INDATA: Cannot find checkpoint data for as_p -- aborting..."
                << endl;
            exit(0);
        }
        return 0.0;
