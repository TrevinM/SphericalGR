...
public:
  ...
  //================================================
  Gen_TOV_Solution(EOS * eos_i, double rho_c, 
		   double dr_init, double N_array_i) :
    eos(eos_i), N_array(N_array_i)
  {
    PI = acos(-1.0);
    //
    // integrate a first time to find radius and mass
    //
    cout << " TOV: Building TOV star for EOS " << eos->Name() << endl;
  ...
  // Destructor
  //================================================
  ~Gen_TOV_Solution() { delete rho_0_array; delete lapse_array; delete psi_array; delete r_array; delete r_iso_array; }
  //================================================
  ...
  double rho_0(double r_bar) {
    if (r_bar > R_iso) {
      ...
      // in interior
      //    
      // cout << " TOV: at r = " << r_bar << " have rho_0 = "
      // << Interpolator(r_bar, rho_0_array) << endl; 
  ...
  void Integrate(double rho_c) {
	...
	if (vars[0] < 0.0) vars[0] = 0.0;
      ...
      // cout << " at r = " << r << " have rho_0 = " << (*rho_0_array)[i]
      // 	   << " P  = " << vars[0]
      // 	   << " lapse = " << exp(vars[2])
      // 	   << " psi = " << (*psi_array)[i]
      // 	   << " r_iso = " << (*r_iso_array)[i]
  ...
  double P_of_rho_0(double rho_0) { 
    double eps = eps_of_rho_0(rho_0);
    return eos->P(rho_0, eps);
  }
  double rho_0_of_P(double P) { return eos->rho_0(P); }
  double eps_of_rho_0(double rho_0) { return eos->cold_eps(rho_0); }
  //================================================
