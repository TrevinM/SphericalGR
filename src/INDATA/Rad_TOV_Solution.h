...
//
// TOV integrator: 
// integrates TOV equations for central rest_mass density rho_i = rho_c,
// then provides TOV solution as function of isotropic radius for fluid
// dominated by radiation pressure - sets rho_0 to zero
//
// In constructor, provide
// - central energy density rho_c (= rho_i at center)
...
private:
  ...
  double PI;
  VecDoub *rho_i_array, *lapse_array, *psi_array, *r_array, *r_iso_array;
public:
  //================================================
  // Constructor: carries out integration and sets up arrays for interpolation
  //================================================
  Rad_TOV_Solution(double rho_c, 
  ...
  // Destructor
  //================================================
  ~Rad_TOV_Solution() { delete rho_i_array; delete lapse_array; delete psi_array; delete r_array; delete r_iso_array; }
  //================================================
  // lapse as function of isotropic radius
  //================================================
  double rho_i(double r_bar) {
    if (r_bar > R_iso) {
      //
      // in exterior
      //    
      ...
      // in interior
      //    
      return Interpolator(r_bar, rho_i_array);
    }
  };
  //================================================
  // lapse as function of isotropic radius
  ...
  void Integrate(double rho_c, double dr_init) {
    ...
    double dr = dr_init;
    double tol = 1.e-6;
    while (vars[0] > tol*rho_c) {
      Derivs(r, vars, d_vars_dr);
      r += dr;
      //      cout << " r = " << r << " rho_i = " << vars[0] << endl;
  ...
  void Derivs(double r, VecDoub f, VecDoub & dfdr) {
    if (r == 0.0) {
      ...
      const double rho_i = 3.0 * P;
      dfdr[0] = - ( rho_i + P ) * (m + 4.0 * PI * r*r*r * P) / ( r*r - 2.0 * m * r);
      dfdr[1] = 4.0 * PI * rho_i * r*r;
      dfdr[2] = - dfdr[0] / (rho_i + P);
  ...
  //================================================
  //  double P_of_rho_i(double rho_i) { return Kappa * pow(rho_i,Gamma); }
  //  double rho_i_of_P(double P) { return pow( P/Kappa, 1.0/Gamma ); }
  //  double eps_of_rho_i(double rho_i) { return Kappa * pow(rho_i,Gamma-1.0)/(Gamma-1.0); }
  //================================================
