...
double Hamiltonian_Residual(bool verbose = false) {
  ...
  for (int i = N_g + 1; i < n_r-N_g; i++)
	...
	for (int k = N_g; k < n_phi-N_g; k++) {
	  ...
	  const double lapsel = lapse_D(i,j,k) + lapse_BH(i,j,k);
	  const double lapse2 = lapsel * lapsel;
	  const double lapseBHl = lapse_BH(i,j,k);
	  const double lapseBH2 = lapseBHl * lapseBHl;
	  const double Kl = K_BH(i,j,k);
	  const double rhol = q(i,j,k);  // FIX THIS!!!
	  const double temp = 1./12.*(psi5 - psiBH5)*Kl*Kl
	    - 1./32.*(psi5*L_beta2(i,j,k)/lapse2 - psiBH5*L_beta_BH2(i,j,k)/lapseBH2 )
	    - 2*PI*psi5pn*rhol;
...
void compute_f() {
    ...
    for (int i = 0; i < n_r; i++)
	  ...
	  const double L_beta2l = L_beta2(i,j,k);
	  const double rhol = q(i,j,k);  // FIX THIS!!!
	  f[i][j][k] = - 5./12.*psi4*Kl*Kl
	    + 5./32.*psi4*L_beta2l/(lapsel*lapsel)
	    + 10.*PI*psi4*rhol;
	}
  };
void compute_psi() {
