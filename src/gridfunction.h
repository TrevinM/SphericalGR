...
public:
  ...
  // utilities...
  //================================================
  void add_v_r(double factor, double shield) {
    for (int i = 0; i < nr; i++)
      for (int j = 0; j < nt; j++)
	for (int k = 0; k < np; k++) {
	  double rho = (grid->r(i)) / shield;
	    v[i][j][k] -= 1. *  factor * tanh(rho) * (grid->costheta(j));
	}
  };
  void add_v_t(double factor, double shield) {
    for	(int i = 0; i < nr; i++)
      for (int j = 0; j < nt; j++)
        for (int k = 0; k < np; k++) {
	  double rho = (grid->r(i)) / shield;
          v[i][j][k] +=	1. * factor * tanh(rho) * (grid->sintheta(j));
	}
  };
  void add_const(double factor) {
    for (int i = 0; i < nr; i++)
      for (int j = 0; j < nt; j++)
	for (int k = 0; k < np; k++) {
	  v[i][j][k] += factor;
