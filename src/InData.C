...
InData::InData(Grid* grid_i, Cosmology* cosmology_i) :
...
void InData::SelfSim(double& tau_star_r, double& xi_r) {
...
// Metric
//================================================  
bool InData::Initialize_Metric(state* c) {
    // cout << "1" << endl;
...
//================================================
void InData::Rescale_Metric(gf3d& h_rr, gf3d& h_rt, gf3d& h_rp,
...
//================================================
void InData::Initialize_ExCurvature(state* c) {
...
void InData::Initialize_Lapse(state* c) {
    const double t = 0.0;
...
// Shift
//================================================
void InData::Initialize_Shift(state* c) {
    const double t = 0.0;
...
// Matter
//================================================
void InData::Initialize_Matter(gf3d& rho_0, gf3d& v_r, gf3d& v_t, gf3d& v_p) {
    const double t = 0.0;
    int N_r = rho_0.dim1();
...
// Radiation Hydro
//================================================
void InData::Initialize_RadHydro(gf3d& rho_0, gf3d& p, gf3d& v_r, gf3d& v_t, gf3d& v_p,
    gf3d& E, gf3d& F_0,
...
// Scalar Field
//================================================
void InData::Initialize_Scalar_Field(gf3d& sf, gf3d& pi) {
    const double t = 0.0;
    int N_r = sf.dim1();
    int N_theta = sf.dim2();
    int N_phi = sf.dim3();
...
//================================================
void InData::Initialize_Maxwell(gf3d& e_p, gf3d& a_p) {
    const double t = 0.0;
    int N_r = e_p.dim1();
    int N_t = e_p.dim2();
    int N_p = e_p.dim3();
...
//================================================
void InData::Initialize_DualMaxwell(gf3d& a_r, gf3d& a_t, gf3d& a_p,
    ...
    for (int i = N_g; i < N_r - N_g; i++) {
        double rl = a_p.r(i);
        for (int j = N_g; j < N_t - N_g; j++) {
            double tl = a_p.theta(j);
            for (int k = N_g; k < N_p - N_g; k++) {
                double pl = a_p.phi(k);
                a_r[i][j][k] = a_r_analytical(rl, tl, pl, t);
                a_t[i][j][k] = a_t_analytical(rl, tl, pl, t);
                a_p[i][j][k] = a_p_analytical(rl, tl, pl, t);
                as_r[i][j][k] = as_r_analytical(rl, tl, pl, t);
                as_t[i][j][k] = as_t_analytical(rl, tl, pl, t);
                as_p[i][j][k] = as_p_analytical(rl, tl, pl, t);
            }
        }
    }
    a_r.fill_ghosts();
...
//       called.
//================================================
void InData::Initialize_ConfConn(state* s, curvature* c) {
...
//================================================
 ...
 //================================================
void InData::Initialize_ConfConn(state* cc) {
    double t = 0.0;
...
bool InData::Analytical_Solution_Exists() {
    return analytical;
}
...
//================================================
int InData::Adjust_Grid(double& r_out, int& N_r, int& N_theta) {
...
//================================================
vect InData::Cartesian_to_Spherical_upper(vect V, Doub r, Doub theta, Doub phi) {
...
//================================================
tensor InData::Cartesian_to_Spherical(tensor g, double r, double theta, double phi) {
    Doub st = sin(theta);
    Doub ct = cos(theta);
    Doub sp = sin(phi);
    Doub cp = cos(phi);
