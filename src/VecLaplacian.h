...
//================================================
//
// Uses Trilinos software to solve covariant so solve
//
//  (\Delta_L b)^i - (L beta)^{ij} u_j = s^i
//
// where b^i, u_i, and s^i are all vectors
//
...
public:
  ...
  // Set right hand side
  //================================================
  int Setu(gf3d & u_grid_r, gf3d & u_grid_t, gf3d & u_grid_p) {
    return Setu(1.0, u_grid_r, u_grid_t, u_grid_p);
  };
  int Setu(double factor,
	     gf3d & u_grid_r, gf3d & u_grid_t, gf3d & u_grid_p) {
    //    cout << " Setting up RHS..." << endl;
    for (int II = 0; II < n_global; II++) { // loop over superindex
      if (II < n_vec) {
	(*u)[II] = factor * u_grid_r(i_ind(II),j_ind(II),k_ind(II));
      } else if (II < 2*n_vec) {
	(*u)[II] = factor * u_grid_t(i_ind(II),j_ind(II),k_ind(II));
      } else {
	(*u)[II] = factor * u_grid_p(i_ind(II),j_ind(II),k_ind(II));
      }
    }
    return 1;
  };
  ...
  int SetInitialGuess(gf3d & guess_r, gf3d & guess_t, gf3d & guess_p) {
    ...
    for (int II = 0; II < n_global; II++) { // loop over superindex
      if (II < n_vec) {
	(*sol)[II] = guess_r(i_ind(II),j_ind(II),k_ind(II));
      } else if (II < 2*n_vec) {
	(*sol)[II] = guess_t(i_ind(II),j_ind(II),k_ind(II));
      } else {
	(*sol)[II] = guess_p(i_ind(II),j_ind(II),k_ind(II));
      }
    }
    return 1;
  };
...
private:
  ...
  int SetupCovLaplace() {
    ...
    for (int ii = 0; ii < NumMyElements; ii++) { // loop over superindex
      ...
      const double dx_uterm     = 1.0/(2.0*d_x)*dxdr;
      const double dy_uterm     = 1.0/(2.0*d_y)*dydtheta/rl; 
      const double dxdy_term    = 1.0/(4.0*d_x*d_y)*dxdr*dydtheta/rl;
      //
      // grab vector components u_i
      //
      double parity = 1.0;
      const double u_r = (*u)[II_ind(i,j,k,R,parity)];
      const double u_t = (*u)[II_ind(i,j,k,THETA,parity)];
      const double u_p = (*u)[II_ind(i,j,k,PHI,parity)];
      //
	...
	double IJ = - (7./3.)*cottheta/r2
	  + (2./3.)*u_r*cottheta/rl + u_t/rl;
	// i-1, j, k, THETA: d_r b^t
	Indices[4] = II_ind(i-1, j, k, THETA, parity);
	Values[4] = parity * ( - cottheta/3.*dx_term + u_t*dx_uterm );
	// i, j-1, k, THETA: d_t b^t prepare only
	double IJm1 = 7./3.*dy_term - (2./3.)*u_r*dy_uterm;
	// i, j+1, k, THETA: d_t b^t prepare only
	double IJp1 = - 7./3.*dy_term + (2./3.)*u_r*dy_uterm;
	...
	// i, j, k, PHI: b^p prepare only
	double PHI_diag = + u_p/rl;
	// i-1, j, k, PHI: d_r b^p
	Indices[7] = II_ind(i-1, j, k, PHI, parity);
	Values[7] = parity * (+ u_p*dx_uterm );
	...
	if (i < n_r - N_g - 1) {   // i+1 is in interior of grid...
	  // i+1, j, k, R: d^2_r b^r, d_r b^r  
	  Indices[8] = II_ind(i+1, j, k, R, parity);
	  Values[8] = parity * ( (4./3.)*(ddx_term + dx2_term) + (8./3.)*dx_term
				 - (4./3.)*u_r*dx_uterm);
	  // i, j, k, THETA: b^t 
	  Indices[9] = II_ind(i, j, k, THETA, parity);
	  Values[9] = parity * IJ;
	  // i+1, j, k, THETA: d_r b^t
	  Indices[10] = II_ind(i+1, j, k, THETA, parity);
	  Values[10] = parity * ( cottheta/3.*dx_term - u_t*dx_uterm );
	  // i+1, j-1, k, THETA: d_r d_t b^t
	  Indices[11] = II_ind(i+1, j-1, k, THETA, parity);
	  Values[11] = parity * ( - dxdy_term / 3.0);
	  // i+1, j+1, k, THETA: d_r d_t b^t
	  Indices[12] = II_ind(i+1, j+1, k, THETA, parity);
	  Values[12] = parity * ( dxdy_term / 3.0);
	  // i, j-1, k, THETA: d_t b^t 
	  Indices[13] = II_ind(i, j-1, k, THETA, parity);
	  Values[13] = parity * IJm1;
	  // i, j+1, k, THETA: d_t b^t 
	  Indices[14] = II_ind(i, j+1, k, THETA, parity);
	  Values[14] =  parity * IJp1;
	  // i, j, k, PHI: b^p
	  Indices[15] = II_ind(i, j, k, PHI, parity);
	  Values[15] = parity * ( PHI_diag );
	  // i+1, j, k, PHI: d_r b^p
	  Indices[16] = II_ind(i+1, j, k, PHI, parity);
	  Values[16] = parity * ( - u_p*dx_uterm );
	  ...
	  //
	  ierr = A->InsertGlobalValues(MyGlobalElements[ii],17,&Values[0],&Indices[0]);
	  assert(ierr==0);
	  ...
	  iterm = cottheta/3.*dx_term - u_t*dx_uterm;
	  IJ += factor * iterm;
	  ...
	  iterm = dxdy_term / 3.0;
	  IJp1 += factor * iterm;
	  Indices[10] = II_ind(i, j+1, k, THETA, parity);
	  Values[10] = parity * IJp1;
	  // i+1, j, k, PHI: corrects PHI_diag term
	  iterm = - u_p*dx_uterm;
	  PHI_diag += factor * iterm;
	  Indices[11] = II_ind(i, j, k, PHI, parity);
	  Values[11] = parity * PHI_diag;
	  // 
	  // now store matrix elements
	  //
	...
	double IJm1 = - 8./3.*dy_term + u_r*dy_uterm;
	// i, j+1, k, R: d_t b^r prepare only
	double IJp1 =   8./3.*dy_term - u_r*dy_uterm;
	...
	Values[5] = parity * ( - dxdy_term / 3.0 );
	// i-1, j, k, R: d_r b^r
	Indices[6] = II_ind(i-1, j, k, R, parity);
	Values[6] = parity * ( - (2./3.)*u_t*dx_uterm );
	// i, j, k, R: b^r prepare only
	double R_diag = - (2./3.)*u_t/rl;
	// i, j, k, PHI: b^p
	Indices[7] = II_ind(i, j, k, PHI, parity);
	Values[7] = parity * ( cottheta * u_p / rl);
	// i, j-1, k, PHI: d_t b^p
	Indices[8] = II_ind(i, j-1, k, PHI, parity);
	Values[8] = parity * (   u_p*dy_uterm );
	// i, j+1, k, PHI: d_t b^p
	Indices[9] = II_ind(i, j+1, k, PHI, parity);
	Values[9] = parity * ( - u_p*dy_uterm );
	...
	if (i < n_r - N_g - 1) {   // i+1 is in interior of grid...
	  // i+1, j, k, THETA: d^2_r b^t, d_r b^t  
	  Indices[10] = II_ind(i+1, j, k, THETA, parity);
	  Values[10] = parity * ( ddx_term + dx2_term + 2.0*dx_term
				 - u_r*dx_uterm );
	  // i+1, j-1, k, R: d_r d_t b^r
	  Indices[11] = II_ind(i+1, j-1, k, R, parity);
	  Values[11] = parity * ( - dxdy_term / 3.0);
	  // i+1, j+1, k, R: d_r d_t b^r
	  Indices[12] = II_ind(i+1, j+1, k, R, parity);
	  Values[12] = parity * ( dxdy_term / 3.0);
	  // i, j-1, k, R: d_t b^r 
	  Indices[13] = II_ind(i, j-1, k, R, parity);
	  Values[13] = parity * IJm1;
	  // i, j+1, k, R: d_t b^r 
	  Indices[14] = II_ind(i, j+1, k, R, parity);
	  Values[14] = parity * IJp1;
	  // i+1, j, k, R: d_r b^r
	  Indices[15] = II_ind(i+1, j, k, R, parity);
	  Values[15] = parity * (   (2./3.)*u_t*dx_uterm );
	  // i, j, k, R: b^r prepare only
	  Indices[16] = II_ind(i, j, k, R, parity);
	  Values[16] = parity * R_diag;
	  // diagonal element:
	  Indices[0] = II;	  
	  Values[0] = diag; 
	  // 
	  ...
	  Values[10] = parity * IJm1;
	  // i+1, j+1, k, R: corrects IJp1 term
	  iterm = dxdy_term / 3.0;
	  IJp1 += factor * iterm;
	  Indices[11] = II_ind(i, j+1, k, R, parity);
	  Values[11] = parity * IJp1;
	  // i+1, j, k, R: corrects R_diag term
	  iterm = (2./3.)*u_t*dx_uterm;
	  R_diag += factor * iterm;
	  Indices[12] = II_ind(i, j, k, R, parity);
	  Values[12] = parity * R_diag;
	...
	//
	double parity = 1.0;
	// i, j k, PHI: diagonal element - prepare only...
	double diag = - 2.0 * (ddx_term + ddy_term) - 1.0/(r2*sin2theta)
	  + u_r/rl + cottheta * u_t/rl;
	...
	Values[2] = parity * ( ddy_term - dy2_term - cottheta*dy_term + u_t * dy_uterm );
	// i, j+1, k, PHI: d^2_t b^p, d_t b^p
	Indices[3] = II_ind(i, j+1, k, PHI, parity);
	Values[3] = parity * ( ddy_term + dy2_term + cottheta*dy_term - u_t * dy_uterm );
	// i, j, k, R: b^r prepare only
	double R_diag = - (2./3.)*u_p/rl;
	// i, j, k, THETA: b^t
	Indices[4] = II_ind(i, j, k, THETA, parity);
	Values[4] = parity * ( - (4./3.)*cottheta*u_p/rl); 
	// i-1, j, k, R: d_r b^r
	Indices[5] = II_ind(i-1, j, k, R, parity);
	Values[5] = parity * ( - (2./3.) * u_p * dx_uterm );
	// i, j-1, k, THETA: d_t b^t
	Indices[6] = II_ind(i, j-1, k, THETA, parity);
	Values[6] = parity * ( - (2./3.)* u_p * dy_uterm ); 
	// i, j+1, k, THETA: d_t b^t
	Indices[7] = II_ind(i, j+1, k, THETA, parity);
	Values[7] = parity * ( + (2./3.)* u_p * dy_uterm ); 
	//
	// rest depends on where i is...
	//
	if (i < n_r - N_g - 1) {   // i+1 is in interior of grid...
	  // i+1, j, k, PHI: d^2_r b^p, d_r b^p  
	  Indices[8] = II_ind(i+1, j, k, PHI, parity);
	  Values[8] = parity * ( ddx_term + dx2_term + 2.0*dx_term
				 - u_r*dx_uterm );
	  // i, j, k, R: b^r 
	  Indices[9] = II_ind(i, j, k, R, parity);
	  Values[9] = parity * R_diag;
	  // i+1, j, k, R: d_r b^r
	  Indices[10] = II_ind(i+1, j, k, R, parity);
	  Values[10] = parity * ( + (2./3.) * u_p * dx_uterm );
	  // diagonal element:
	  Indices[0] = II;	  
	  Values[0] = diag; 
	  // 
	  ...
	  Indices[0] = II;	  
	  Values[0] = diag;
	  // i+1, j, k, R: corrects R_diag
	  iterm = + (2./3.) * u_p * dx_uterm;
	  R_diag += factor * iterm;
	  Indices[8] = II_ind(i, j, k, R, parity);
	  Values[8] = parity * R_diag;
	  // 
	  // now store matrix elements
	  //
